...
class BertEmbeddings(nn.Module):
    """从单词/位置等信息进行词嵌入（采用的bert模型更有利于文本特征的获取
    """
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # "query" 和"key" 点乘操作获得初始注意力机制的分数值。
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # 在bert模型前向传播中都提前加入注意力机制参与运算

        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)

        # attention_mask is 1.0 所以要创建个张量0.0，给要加入的位置，-100000表示标记位置，为了后面softmax函数的处理
...
class GeneraProb(nn.Module):       #生成概率
    def __init__(self,config):
...
class Decoder(nn.Module):
    ...
    # decoder_input_one_step (batch,t)   (t = step +1)（时间步加一）
    # decoder_status = (h_t,c_t)  h_t (1,batch,hidden)
    # encoder_output (batch,seq_len,hidden*2)
    # encoder_mask (batch,seq_len)
    # context_vec (bach,hidden*2)  注意力得分的编码器权重和
    # oovs_zero (batch,max_oov_size)  全零张量
    # encoder_with_oov (batch,seq_len)  oov中的单词索引处理
    # coverage : 每个步骤的注意力总数
    # step...
            ...
            for beam in sort_beams(all_beams):
                if beam.tokens[-1] == self.config.stop_idx:
                    if len(beam.tokens) > self.config.min_title_len:
                        ...
                        pass          # 把beam_size适当调大，min_decoder_len适当调小
                else:
                    beams.append(beam)
